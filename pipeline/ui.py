...
def run_pipeline_subprocess(mapping_path, work_dir, s3_bucket, s3_prefix, status_placeholder, control):
    # Use a scrollable text area for logs via the placeholder
    status_placeholder.text_area("Logs", value="Starting pipeline subprocess...\n", height=300)
    cmd = [
        shlex.quote("python"),
        "-u",
        "-m",
    ...
    try:
        # Start subprocess
        proc = subprocess.Popen(shell_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
        control['proc'] = proc
        ...
        if not control.get('cancel'):
            ...
            try:
                ...
                status_placeholder.text_area("Logs", value=current + "\nPipeline finished. Check the work directory for outputs.", height=300)
...
def main():
    ...
    if run_btn:
        ...
        thread = threading.Thread(target=run_pipeline_subprocess, args=(mapping_to_use, work_dir, s3_bucket or None, s3_prefix or '', status, control))
        thread.start()
        ...
        # run a lightweight waiter thread that calls show_artifacts when the subprocess clears
        def waiter():
        ...
        try:
            ...
            if std_dir.exists():
                ...
                zip_buf.seek(0)
                st.download_button("Download standardized artifacts (ZIP)", data=zip_buf, file_name="standardized_artifacts.zip")
        except Exception:
            pass
